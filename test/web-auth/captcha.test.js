...
describe('captcha rendering', function () {
  ...
  describe('when challenge is required and the provider is authok', function () {
  ...
  describe('when challenge is required, the provider is authok and we use a custom template', function () {
    const button = {
      listeners: {},
    ...
    beforeEach(function () {
      ...
      captcha.render(mockClient, element, {
        templates: {
          authok: challenge => `custom template: ${challenge.image}`
        }
      });
    });
  ...
  [RECAPTCHA_V2_PROVIDER, RECAPTCHA_ENTERPRISE_PROVIDER].forEach(provider => {
    ...
    describe(`when challenge is required and provider is ${provider}`, function () {
      ...
      beforeEach(() => {
        ...
        ].find(s => s.src.match('recaptcha.net'));
        scriptOnLoadCallback =
          window[url.parse(recaptchaScript.src, true).query.onload];
      });
