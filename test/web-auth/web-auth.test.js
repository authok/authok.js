...
describe('authok.WebAuth', function () {
  ...
  context('parseHash', function () {
        ...
        function (err, data) {
          ...
          expect(data).to.be.eql({
            ...
            idTokenPayload: {
              ...
              picture:
                'https://s.gravatar.com/avatar/38fa002423bd8c941c6ed0588b60ffed?s=480&r=pg&d=https%3A%2F%2Fcdn.authok.cn%2Favatars%2Fjo.png',
              updated_at: '2018-03-14T16:34:56.423Z',
              email: 'johnfoo@gmail.com',
              email_verified: false,
              iss: 'https://brucke.authok.cn/',
        ...
        webAuth.parseHash(
          ...
          function (err, data) {
            expect(err).to.eql({
              error: 'invalid_token',
              errorDescription:
                'Issuer (iss) claim mismatch in the ID token, expected "https://wptest_2.authok.cn/", found "https://wptest.authok.cn/"' // eslint-disable-line
            });
            done();
          }
        ); // eslint-disable-line
    ...
    context('with HS256 id_token', function () {
      beforeEach(function () {
        this.webAuth = new WebAuth({
          domain: 'authok-tests-lock.authok.cn',
          redirectUri: 'http://example.com/callback',
      ...
      sinon
        ...
        .callsFake(function () {
          expect(this.authenticationUrl).to.be(
            'https://me.authok.cn/authorize?client_id=...&response_type=id_token&redirect_uri=http%3A%2F%2Fpage.com%2Fcallback&scope=openid%20name%20read%3Ablog&audience=urn%3Asite%3Ademo%3Ablog&nonce=123&state=456&response_mode=fragment&prompt=none'
          );
          done();
        });
  ...
  context('authorize', function () {
    ...
    it('should default scope to openid profile email', function (done) {
      ...
      sinon.stub(windowHelper, 'redirect').callsFake(function (url) {
        expect(url).to.be(
          'https://me.authok.cn/authorize?client_id=...&response_type=token&redirect_uri=http%3A%2F%2Fpage.com%2Fcallback&connection=foobar&state=randomState&scope=openid%20profile%20email'
        );
  ...
  context('signup', function () {
    ...
    it('should call db-connection signup with all the options', function (done) {
      sinon.stub(request, 'post').callsFake(function (url) {
        if (url === 'https://me.authok.cn/oauth/token') {
          return new RequestMock({
            body: {
              client_id: '...',
              realm: 'the_connection',
              grant_type: 'http://authok.cn/oauth/grant-type/password-realm',
    ...
    it('inits IframeHandler with correct params', function (done) {
      sinon.stub(IframeHandler.prototype, 'init').callsFake(function () {
        expect(this.url).to.be(
          'https://me.authok.cn/authorize?client_id=...&response_type=token&redirect_uri=http%3A%2F%2Fpage.com%2Fcallback&from=transaction-manager&response_mode=web_message&prompt=none'
        );
